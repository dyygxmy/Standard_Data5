...
void FtpJson::FindFile(QString path)
{
    ...
    do{
        ...
        {
            //nFiles++;
            // DTdebug() << QString("%1 %2 %3").arg(fileInfo.size(), 10)
           // DTdebug() << "find file2";
        ...
        {
            DTdebug()<<"file文件名"<<fileInfo.filePath() ;
            DTdebug()<<fileInfo.filePath() <<"file创建时间"<<fileInfo.created() ;
            DTdebug()<<fileInfo.filePath() <<"file最后修改时间"<<fileInfo.lastModified() ;
            //nFiles++;
            // DTdebug() << QString("%1 %2 %3").arg(fileInfo.size(), 10)
            // .arg(fileInfo.fileName(),10).arg(fileInfo.path())<<endl;
           // DTdebug() << "find file parsing!!";
...
//                    unlink(fileInfo.absoluteFilePath().toLocal8Bit().data());
                    ...
                    //todo
                    DTdebug() << "file "+fileInfo.fileName()<<" VIN match fail";
                }
            }
            else
                parseFile(fileInfo.absoluteFilePath(),fileInfo.fileName());
...
//            exit (1);
            system(QString("mv "+ filepath+" /tmp/wrongCurve/").toLocal8Bit().data());   // todo
...
//        DTdebug() << "id code:" << result["id code"].toString().mid(0,17);
    ...
    //    DTdebug()<< size;
        ...
        if(screwid[Channel.toInt()-1] != "")
        {
            ...
            DTdebug()<<"emit date_model:"<<demo.data_model[0]<<demo.data_model[1]<<demo.data_model[2]<<demo.data_model[3]<<demo.data_model[4]<<demo.data_model[5]<<demo.data_model[6]<<demo.data_model[7]<<demo.data_model[9]<<demo.data_model[10];
            QVariant DataVar;
            ...
            else if(screwid[Channel.toInt()-1]=="200000000"||screwid[Channel.toInt()-1]=="300000000")
            {
                ...
                system(QString("mv "+ filepath+" /tmp/wrongCurve/").toLocal8Bit().data());      //  todo
                DTdebug() << "match fail move file "+filepath+" to /tmp/wrongCurve";
            }
            else if(Program =="99")
            {
                unlink(filepath.toLocal8Bit().data());
                DTdebug() << "delete file Program 99 "+filepath;
            }
            else
            {
                unlink(filepath.toLocal8Bit().data());
        ...
        {
//            DTdebug() << "What are you ? "<< "file: "+fileName;
//            unlink(filepath.toLocal8Bit().data());
            system(QString("mv "+ filepath+" /tmp/wrongCurve/").toLocal8Bit().data());       // todo
            DTdebug() << "screwid match fail move file "+filepath+" to /tmp/wrongCurve";
        }

    }
    else
    {
        DTdebug() << filepath <<" is incomplete or wrong,right10:"<<json.simplified().right(10);
        QMap<QString,int>::iterator iter;
        bool newfile = true;
        for(iter=wrongMap.begin(); iter != wrongMap.end(); ++iter)
        {
...
//                wrongMap[filepath]++;
                ...
                {
                    wrongMap.erase(iter);
                    DTdebug()<<"delete "+filepath+" from wrongMap >7 times" << json.simplified().right(10);
                    system(QString("mv "+ filepath+" /tmp/wrongCurve/").toLocal8Bit().data());       // todo
//                    unlink(filepath.toLocal8Bit().data());
