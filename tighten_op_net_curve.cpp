...
{
    DTdebug() << "tighten_op_net thread start!!!";
...
void TightenOpNetCurve::sendReadOperate(bool enable,int n)
{
    ...
    else
    {
        ...
        {
            sendVinFlag = 0;
            DTdebug() << "sendVinFlag=0";
        }
        else
        {
            vinBuf = VIN_PIN_SQL.toLatin1();
            sendVinFlag = 1;
            DTdebug() << "sendVinFlag=1";
        }
        if(nexoLinkOk)
        {
            timer5000ms->stop();
...
{
    recBufBk.append(tcpSocket->readAll());

    while(recBufBk.size() >= 8)
    {
        bool okFlag;
        int nexoMID;
        int cmdType;
        ...
        nexoMID = recBufBk.mid(4,4).toInt();
        revLen = recBufBk.size();
        cmdLen = recBufBk.mid(0,4).toInt();
        DTdebug() << "recevice len:" << revLen << cmdLen;
        ...
        recBuf = recBufBk.mid(0, (cmdLen+1));
        recBufBk = recBufBk.mid((cmdLen+1));
        revLen = recBuf.size();
        DTdebug() << "recevice frame : " << revLen << cmdLen << recBuf;
        if(!recBufBk.isEmpty())
        {
            DTdebug() << "extend frame : " << recBufBk.size() << recBufBk;
        }
            ...
            case 61://Rev result data
                ...
                if(SYSS == "ING")
                {
...

                    {
                        ...
                        memcpy((char*)&tmpData, recBuf.data(), qMin((int)sizeof(T_MID61_DATA_V2), recBuf.size()));
                        ...
                        int tmpFinalAngle = QString::fromLatin1((char*)(tmpData.mAngle+2), 5).toInt();
                        ...
                        DTdebug() << "tmpFinalAngle" << tmpFinalAngle;
                        DTdebug() << "tmpBinaryOffset" << tmpBinaryOffset;
                        DTdebug() << "tmpTorqueScaleFactor" << tmpTorqueScaleFactor;
                        int tmpTraceStartIndex;
                        int tmpNewTraceLength;
                        if((tmpAngleOffset > 4500) && (tmpTraceLength == 4500))
                        {
                            tmpTraceStartIndex = tmpAngleOffset+tmpFinalAngle-4500;
                            tmpNewTraceLength = tmpAngleOffset+tmpFinalAngle+1;
                        }
                        else
                        {
                            tmpTraceStartIndex = 0;
                            tmpNewTraceLength = tmpTraceLength;
                        }
                        for(int i=0;i<tmpTraceLength;i++)
                        {
                           ...
                           // DTdebug() << "tmpDataBlobValue" << tmpDataBlobValue;
                            double tmpTorqueValue = (double)(tmpDataBlobValue-tmpBinaryOffset)/tmpTorqueScaleFactor;
                           // DTdebug() << "tmpTorqueValue" << tmpTorqueValue;
                            tmpTorqueList.append(QString::number(tmpTorqueValue,'f',2));
                        }
                        for(int i=tmpTraceStartIndex;i<tmpNewTraceLength;i++)
                        {
                            tmpAngleList.append(QString::number(-tmpAngleOffset+i));
                        }
    ...
    //                tighteningID = recBuf.mid(60,9);                           //nexo Tightening ID
                    ...
                    DTdebug() << "****************tighteningID" << tighteningID;
