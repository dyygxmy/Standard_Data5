...
{
    DTdebug()<< "xml data thread start"<<vari1;
...
void xmlData::xml_parse(QByteArray temp)
{
    ...
    DTdebug() << "xml temp" << temp.mid(32);

    dY1V = 0 ;
    dXVA = 0;

//    QFile file1("/atlas.xml");
    QFile file("/a.xml");
    //新建文件并保存
    ...
    //DTdebug() << node.nodeName()<< "nodename";
    if(node.nodeName() == "PRC_SST")
    {
        //曲线
        CurveStr = temp;
        vinCode  = 1;
        DTdebug() << " PRC_SSTPRC" <<CurveStr;
...
//                                            }
        ...
        if(Program != "-1")
        {
            ...
            if(WI_blc  == "")
                WI_blc = "0";
            DTdebug() << "xml Thread parse data:" << vinCode+"||"+Cycle_CSR+"||"+MI_blc+"||"+WI_blc+"||"+Program+"||"+Channel;
