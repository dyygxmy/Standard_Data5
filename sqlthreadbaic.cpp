...
SqlThreadBAIC::SqlThreadBAIC(QObject *parent) :
    QObject(parent)
{
    m_thread.start();
...
//    DATA_STRUCT tmpData;
//    for(int i=0;i<19;i++)
//        tmpData.data_model[i] = QString::number(i);
//    QVariant DataVar;
//    DataVar.setValue(tmpData);
//    sqlinsert(DataVar);
}
/*
     数据库关闭
*/
void SqlThreadBAIC::sqlclose()
{
    //     if(db1.isOpen())
    //       db1.close();
    if(db2.isOpen())
...
void SqlThreadBAIC::mysqlopen()
{
    ...
    if(!db2.open())
    {
        if(!db2.open())
        {
            DTdebug()<< "SqlThreadBAIC localmysql "<< db2.lastError().text();
            //**************打不开重启mysql********************
            emit send_mysqlerror();

        }else
        {
            DTdebug()<< "SqlThreadBAIC localmysql ok 2";
        }
    }else
    {
        DTdebug()<< "SqlThreadBAIC localmysql ok 1";
    }
}

void SqlThreadBAIC::sqlinsert(QVariant DataVar)
{
    //return;
...
//    int Channel = data_model[10].toInt();
    ...
    if(screwid_sql.isEmpty())
    {
        msg= QString("Screwid=0 discard the data : IDCode:")+data_model[6]+QString("||ScrewID:")+screwid_sql+QString("||Torque:")+data_model[3]+
                ...
                ("||LocalDateTime:")+QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss ddd")+QString("||Operator:")+userID+QString("||UploadMark:")+QString::
    ...
    if(db2.isOpen() && QSqlDatabase::contains("mysqlconnections"))
    {
        if(isFirst)
        {       //
            if(!query2.exec("CREATE TABLE IF NOT EXISTS "+Localtable+" (RecordID int not null primary key auto_increment, IDCode varchar(128) not null,ScrewID varchar(28) null,Torque real not null,Angle real not null,Curve longtext not null,TighteningStatus char(3) not null,TighteningTime datetime not null,LocalDateTime datetime not null,Operator varchar(15) null,UploadMark tinyint not null,UploadTime datetime null,Cycle int null,Program int null, Channel int null, Body_NO varchar(20) not null, Order_id int null, `MaxValue` float null, `MinValue` float null, JobStatus int null, MaxAngle float null, MinAngle float null)"))
            {
        ...
        //检测循环号有没重复的，有重复的不上传
        query2.exec("SELECT Cycle FROM "+Localtable+" where Cycle = '" + QString::number(data_model[7].toInt()) + "'");//database存储的Cycle是Int类型
        int numRows_cycle = query2.numRowsAffected();
        if(numRows_cycle > 0){
//            query2.next();
            qDebug()<<"This Cycle is duplicate";
            uploadmark = 8;
        }
        ...
        if (numRows < 10000)
        {
            query2.prepare("INSERT INTO "+Localtable+"(RecordID, IDCode, ScrewID, Torque, Angle, Curve, TighteningStatus, TighteningTime, LocalDateTime, Operator, UploadMark, UploadTime, Cycle, Program, Channel, Body_NO, Order_id, `MaxValue`, `MinValue`, JobStatus, MaxAngle, MinAngle)"   //modify
                           "VALUES (?, ?, ?, ?, ?, ?, ?, ?, now(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        }
        else
        {
        ...
        if(data_model[8].isEmpty() || data_model[8].isNull() || data_model[8] == "null")
            query2.addBindValue("Curve is null");
        else
        {
            int tmpLen = data_model[8].size();
            DTdebug()<<"Curve size is " << tmpLen;
        ...
        query2.addBindValue(tmpCurDateTime);  //"1970-01-01 00:00:01";modify
        ...
        query2.addBindValue(data_model[17].toDouble()); //MinAngle

        bool inserttmp = query2.exec();
        int numRowsAffected=query2.numRowsAffected();

        QString tempMsg = "IDCode:"+data_model[6]+QString("||ScrewID:")+screwid_sql+QString("||Torque:")+data_model[3]+QString("||Angle:")+data_model[4]
                +QString("||TighteningStatus:")+data_model[2]+QString("||TighteningTime:")+data_model[0]+QString("||")+data_model[1]+
                QString("||LocalDateTime:")+QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss ddd")+QString("||Operator:")+userID
        ...
        if(!inserttmp)
        {
            ...
            if(!query2.exec())
            {
                ...
                {
                    //插入3次不成功存文件
                    msg= "SqlThreadBAIC mysql insert false: "+tempMsg;
                    qDebug()<<msg;
                ...
                {
                    msg= "SqlThreadBAIC mysql insert success3: "+tempMsg;
                    qDebug()<<msg;
                }
            }
            else
            {
                msg= "SqlThreadBAIC mysql insert success2: "+tempMsg;
                qDebug()<<msg;
            }
        }
        ...
        {
            msg= "SqlThreadBAIC mysql insert success1: "+tempMsg;
            qDebug()<<msg;
        }
        sqlclose();
    }
...
/***********************************************/
void SqlThreadBAIC::receiveNokAll(int ch)
{
...
void SqlThreadBAIC::configOne(QString screwid, QString vin_pin_sql, QString protmp,int Channel)
{
    Q_UNUSED(Channel);
...
/***********************************************/
void SqlThreadBAIC::configOneGroup(QString vin_pin_sql,QString *screwid,QString *proNO)
{
    int i;
